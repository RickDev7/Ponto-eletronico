...
function entityHref(slug: string, entityType: string, entityId: string): string | null {
  switch (entityType) {
    case "task":
    ...
    case "client":
      return `/${slug}/clients`;
    case "employee":
      return ROUTES.workforceEmployees(slug);
    case "address":
...
export async function loadActivityPageData(companySlug: string, companyId: string, page = 1) {
  ...
  const [
    ...
    supabase
      .from("activity_logs")
      .select(
        `id, action, metadata, created_at, entity_type, entity_id, user_id, profile:profiles(full_name)`,
        { count: "exact" },
      )
    ...
    supabase
      ...
      .select(`
        ...
        task:tasks(id, title, address:addresses(latitude, longitude))
      `)
      .eq("company_id", companyId)
  ...
  const auditsHref = ROUTES.analyticsOperational(companySlug, { tab: "audits" });
  ...
  const failedAuditAlerts: ActivityAlert[] = (auditCheckIns ?? [])
    .flatMap((ci) => {
      ...
      // eslint-disable-next-line @typescript-eslint/no-explicit-any
      const employee = Array.isArray(ci.employee) ? (ci.employee[0] as any) : (ci.employee as any);

      const hasGps = ci.check_in_latitude != null && ci.check_in_longitude != null;
      if (!hasGps || address?.latitude == null || address?.longitude == null) return [];
      ...
      return [
        {
          ...
          href: task?.id ? `/${companySlug}/tasks/${task.id}` : auditsHref,
  ...
  (upcomingTasks ?? []).forEach((task) => {
    ...
    assignments.forEach((assignment) => {
      ...
      if (existing) existing.tasks.push(task.title);
      else {
        conflictMap.set(key, {
  ...
  const completedMonthly = monthlyTasks?.filter((row) => row.status === "completed").length ?? 0;
  const completionRate = totalMonthly > 0 ? Math.round((completedMonthly / totalMonthly) * 100) : 0;
